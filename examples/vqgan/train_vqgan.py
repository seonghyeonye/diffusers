...
def main():
    ...
    # elif args.pretrained_model_name_or_path is not None:
        ...
        print("loading from pretrained ckpt 2!")
        model = VQModel.from_pretrained('amused/amused-256',subfolder="vqvae")
        config = VQModel.load_config('amused/amused-256',subfolder="vqvae")
        model_rand = VQModel.from_config(config)
        model.decoder.load_state_dict(model_rand.decoder.state_dict())
        model.post_quant_conv.load_state_dict(model_rand.post_quant_conv.state_dict())


        # model.encoder.requries_grad=False
        for name, param in model.named_parameters():
            print("name", name)
            param.requires_grad = False
        for p in model.decoder.parameters():
            p.requires_grad = True
        for p in model.post_quant_conv.parameters():
            p.requires_grad = True
    # else:
    #     print("loading from pretrained ckpt!")
    ...
    optimizer = optimizer_cls(
        ...
        list(model.decoder.parameters()) + list(model.post_quant_conv.parameters()),
